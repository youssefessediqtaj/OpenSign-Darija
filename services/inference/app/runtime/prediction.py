...
from app.runtime.model_loader import ModelLoader, model_loader
...
class PredictionService:
    def __init__(self, loader: ModelLoader | None = None) -> None:
        ...
        self._capacity = BoundedSemaphore(get_settings().inference_max_concurrent_requests)
    ...
    def predict_word(self, payload: WordLandmarkSequenceRequest) -> PredictionResponse:
        ...
        if not self._capacity.acquire(blocking=False):
            raise RuntimeError("inference concurrency limit reached")
        try:
            predictions, decision, confidence_level, unknown_probability = (
                self.loader.model.predict(payload)
            )
        finally:
