#!/usr/bin/env python3
"""Snapshot and compare assets protected by structural refactor contracts.
...
The script reads data/model assets only and writes architecture plus root-structure
audit reports. Documentation/report files may change during documentation refactors, so
the primary parity flag focuses on protected model/data/MediaPipe/canonical inference
outputs.
"""
...
ROOT_STRUCTURE_SNAPSHOT_PATHS = {
    "before": ROOT / "artifacts" / "reports" / "root-structure-protected-assets-before.json",
    "after": ROOT / "artifacts" / "reports" / "root-structure-protected-assets-after.json",
}
ROOT_STRUCTURE_PARITY_PATH = ROOT / "artifacts" / "reports" / "root-structure-parity.json"
CANONICAL_FIXTURE = (
    DATASET_ROOT
    / "processed"
    / "landmarks"
...
ADDITIVE_ARTIFACT_REPORTS = {
    ...
    "root-structure-parity.json",
    "root-structure-protected-assets-after.json",
    "root-structure-protected-assets-before.json",
...
def comparison(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    before_mediapipe_tasks = {
        path: metadata
        for path, metadata in before["mediapipe_assets"].get("files", {}).items()
        if path.endswith(".task")
    }
    after_mediapipe_tasks = {
        path: metadata
        for path, metadata in after["mediapipe_assets"].get("files", {}).items()
        if path.endswith(".task")
    }
    protected_keys = {
        ...
        "mediapipe_assets_unchanged": (
            before_mediapipe_tasks == after_mediapipe_tasks
        ),
    ...
    return {
        ...
        **documentation_keys,
        ...
        "all_protected_assets_and_outputs_unchanged": all(protected_keys.values()),
        "documentation_and_report_files_unchanged": all(documentation_keys.values()),
    }
...
def main() -> None:
    ...
    snapshot_payload = {
        "schema_version": "OPEN_SIGNE_PROTECTED_ASSETS_SNAPSHOT_V1",
        "phase": args.phase,
    ...
    for path in (
        PROJECT_STRUCTURE_SNAPSHOT_PATHS[args.phase],
        ROOT_STRUCTURE_SNAPSHOT_PATHS[args.phase],
    ):
        path.write_text(
    ...
    if args.phase == "after":
        ROOT_STRUCTURE_PARITY_PATH.write_text(
            json.dumps(
                {
                    "schema_version": "OPEN_SIGNE_ROOT_STRUCTURE_PARITY_V1",
                    "captured_at": current["captured_at"],
                    "comparison": report["comparison"],
                    "before": report["before_refactor"],
                    "after": report["after_refactor"],
                },
                indent=2,
                ensure_ascii=False,
            )
