import { normalizeSchemaV1Frame } from './landmark-schema-v1-normalizer.service';
...
export type AutomaticSegmentationConfig = {
  targetFrames: number;
...
export function frameMotionEnergy(previous: HolisticFrame | null, current: HolisticFrame): number {
...
export function sequenceDescriptor(frames: HolisticFrame[]): number[] {
...
export function sequenceSimilarity(left: number[], right: number[]): number {
